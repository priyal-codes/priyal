...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return constructMetadata({
    title,
    description: description || `Read ${title} by Priyal Ramteke`,
    keywords: keywords.length > 0 ? keywords : ["Priyal Ramteke", "Blog"],
    image: coverUrl,
    path: `/blogs/${slug}`,
    type: "article",
    publishedTime: date,
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const articleJsonLd = generateArticleJsonLd({
    ...
    description:
      post.meta.description || `Read ${post.meta.title} by Priyal Ramteke`,
